...
const { FiHeart, FiInfo, FiAward, FiStar, FiShield, FiTrendingUp, FiUsers, FiHome, FiActivity } = FiIcons;

const BreedCard = ({ breed, rank, score, isDarkMode = false, onToggleFavorite, isFavorite = false }) => {
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-10">
        <motion.div
          ...
          className={`flex items-center gap-2 px-3 py-1 rounded-full bg-gradient-to-r ${badge.color} text-white text-sm font-bold shadow-lg`}
        >
      ...
      <div className={`relative p-6 bg-gradient-to-r ${getScoreGradient(score)}`}>
        <div className="flex items-center justify-between text-white">
          <div className="flex items-center gap-4">
            <div className="text-6xl">{getBreedIllustration(breedName)}</div>
            <div>
              <h3 className="text-2xl font-bold mb-1">{breedName}</h3>
              <p className="text-white/80 text-sm">{breed.breed_group}</p>
            </div>
          </div>
          <div className="text-right">
            <div className="text-3xl font-bold">{score}%</div>
            <div className="text-white/80 text-sm">{badge.label}</div>
          </div>
        </div>
      ...
      <div className="p-6 space-y-6">
        ...
        >
          <div className="flex items-center justify-center gap-2">
            ...
            {isExpanded ? t('action.show_less', 'Show Less') : t('action.learn_more', 'Learn More')}
          </div>
        </motion.button>
